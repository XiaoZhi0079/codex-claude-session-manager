...
`state_N.sqlite` 和旧索引可以提供标题、供应商和可见性信息，JSONL 或可验证备份负责提供正文。`thread_history_N.sqlite` 是 Codex 从 rollout 物化的分页历史；修改正文后工具会仅清除目标会话的投影行，让 Codex 下次打开该会话时重建。工具不会把只有标题、没有可恢复正文的记录伪装成完整会话。
...
- 恢复前可直接查看备份中的精简会话内容和当前状态差异；单会话备份无需重复选择。
- 支持把可恢复会话调整为当前 Codex 供应商可见。

#### 可见性修复与回退边界

“可见性修复”用于 Codex 版本或供应商切换后，rollout/SQLite 中的 `model_provider` 不一致导致会话不显示的情况。它只统一供应商元数据，并可从已知备份补回缺失的 rollout；它不会转换历史内容，也无法让另一个供应商解密原供应商写入的 `encrypted_content`。因此“重新显示”与“能够跨供应商继续对话”是两件事。

每次实际修复前都会在“系统安全备份”中保存待改 rollout、SQLite 一致性快照和逐项变更清单。回退采用字段级合并，而不是把旧数据库或旧 rollout 整体覆盖回来：

- 修复后新增的会话完全不在回退清单中，不会被删除或修改。
- 已修复会话后来追加的消息会保留；工具只改其 `session_meta.model_provider`。
- SQLite 中后来更新的标题、时间等其他字段会保留；工具只改清单中对应行的 `model_provider`。
- 修复时从备份补回的 rollout 不会在回退时删除，以免丢失后来追加的消息；只回退其供应商字段。
- 若目标 rollout/SQLite 行已删除、路径不安全，或供应商字段后来又被改成第三个值，预览会列出冲突并禁止整批回退。
- 真正回退前还会保存一份当前状态安全点。多个可见性备份不会自动删除；需要释放空间时必须在备份管理中明确永久删除。

例如：周一把旧会话 A 从供应商 P 修复到 Q，之后又在 Q 下继续了 A，并新建会话 B；周五回退周一的修复时，B 保持不变，A 的周一至周五消息也保留，只有 A 的供应商元数据回到 P。此时 A 包含跨供应商时期的历史，P 是否能继续读取或续聊仍取决于 Codex 和供应商的兼容性，工具不作保证。
...
- 删除或修改前创建备份或安全点。
- Codex 会话写入期间持有目标会话的 writer lock，并备份受影响的分页历史数据库。
- SQLite 修改使用事务。
- 文件写入尽量使用原子替换。
- 写入失败时尝试恢复原始内容。
- 操作结果写入本地操作历史。
...
| 恢复 Codex 会话删除备份或轮次快照 | 其他窗口可以；目标会话必须关闭 | 打开目标会话时重建历史 |

工具通过 `%CODEX_HOME%\thread-writer-locks` 判断所选 Codex 会话是否仍在使用。编辑、清理、删除和恢复只要求关闭目标会话，其他 Codex 窗口可保持打开；工具会在整个写入期间继续持有目标锁，防止它被同时重新打开。供应商可见性修复会修改跨会话状态，仍要求完全退出 Codex。
...
备份管理打开期间可以每 15 秒执行一次只读变化检测。它只更新备份清单和恢复预览，不会自动把备份写回会话；任何恢复仍需显式输入 `RESTORE`。
...
点击页面中的“刷新”。如果刚执行过会话恢复或删除，请刷新或重新打开对应客户端；如果执行的是供应商可见性修复，请完全退出并重新打开 Codex，然后再次刷新。
